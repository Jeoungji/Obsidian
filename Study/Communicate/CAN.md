...
- CAN Message Frame
    ...
    |2.0A Data Frame||기능|
    |---|---|---|
    |SOF (Start OF bit)|SOF|메시지의 전송 시작|
    |Falling Edge를 기준으로 버스에 있는 다른 노드들이 동시의 비트 길이 계산|||
    |Hard Synchronization|||
    |Arbitration Phase|ID||
    |RTR|메시지 ID가 작을 수록 우선 순위가 높음||
    |Control Field|IDE||
    |r|||
    |DLC|IDE : Standard Frame인지 Extended Frame인지 확인||
    |DLC : 통신 대역폭을 절약을 위해 유연하게 데이터 필드의 길이를 조절|||
